...
- iPhone/Safari cold offline reopening is not validated. WebKit automation on both the local Windows host and GitHub’s Linux runner fails navigation after going offline despite an active controller and complete cache; this cold-start test is explicitly marked pending, not passed. Downloading, in-page offline score entry, stale-tab protection, sync and sign-out are separately tested in WebKit. Keep the guide open before losing signal until physical-iOS validation is complete.
