...
For public accounts, add the production origin and `/auth/callback` URL to Supabase Auth's allowed redirect URLs, configure a production SMTP sender, and test signup confirmation and recovery in staging. FlightForge accepts a Supabase session only after `email_confirmed_at` is present. A verified hosted email that matches an existing D1 account is sent through explicit identity linking; it is never linked from the email string alone. The PostgreSQL migration journal includes the Supabase auth subject, community tables, curated channels, and private RLS policies.

The FlightForge Supabase project was initialized on August 8, 2026 with PostGIS, the security foundation, and the original reviewed-course seed. That post-seed verification returned 135 courses, 135 locations, 135 primary source rows, 71 evidence rows, 7 roles, and 9 feature flags. The generator invoked by `npm run seed:generate:supabase` now emits all 177 current public listings; the 42 new rows must still be applied and verified during a controlled rehearsal. The public project URL and publishable key are configured in the hosted environment. `DATABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` remain intentionally unset until they can be transferred through a server-secret channel and the full repository parity rehearsal passes.
...
| `NEXT_PUBLIC_SUPPORT_EMAIL` | Public signup | Monitored account and terms support address |
| `NEXT_PUBLIC_PRIVACY_EMAIL` | Public signup | Monitored privacy-request address |
| `LEGAL_TERMS_VERSION` | Public signup | Stable identifier for the reviewed Terms version accepted at signup |
| `LEGAL_PRIVACY_VERSION` | Public signup | Stable identifier for the reviewed Privacy Notice version acknowledged at signup |
