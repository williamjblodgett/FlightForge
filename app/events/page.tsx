...
function EventCard({ event }: { event: EventRecord }) {
  return <article className={`public-event-card ${event.status === "CANCELLED" ? "is-cancelled" : ""}`}>
    <div className="event-date-block"><span>{new Intl.DateTimeFormat("en-US", { month: "short", timeZone: event.timeZone }).format(new Date(event.startsAt))}</span><strong>{new Intl.DateTimeFormat("en-US", { day: "2-digit", timeZone: event.timeZone }).format(new Date(event.startsAt))}</strong></div>
...
function formatDate(value: string, timeZone: string) { return new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric", year: "numeric", timeZone }).format(new Date(value)); }
