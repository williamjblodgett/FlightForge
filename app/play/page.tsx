...
import { CalendarDays, ChevronRight, Clock3, Crosshair, LocateFixed, MapPinned, PlayCircle, RotateCcw } from "lucide-react";
  ...
  if (!eventId) {
    ...
    return <main className="play-page page-shell">
      ...
      <section className="play-start-options" aria-labelledby="start-round-title">
        ...
        <Link className="fieldwork-entry-card" href="/fieldwork">
          <span className="fieldwork-entry-icon"><Crosshair aria-hidden="true" /></span>
          <span className="fieldwork-entry-copy"><small>Open-space practice</small><strong>Fieldwork</strong><span>Find nearby public parks and other course leads open to the public, then estimate throw distance with GPS.</span></span>
          <span className="fieldwork-entry-meta"><LocateFixed aria-hidden="true" /> Uses your location only when you ask</span>
          <span className="fieldwork-entry-action">Open Fieldwork <ChevronRight aria-hidden="true" /></span>
        </Link>
        <div className="play-option-grid"><Link className="play-option-card is-primary" href="/events"><CalendarDays aria-hidden="true" /><span><strong>Event scorecard</strong><small>Open a tournament, league, or casual event.</small></span><ChevronRight aria-hidden="true" /></Link><Link className="play-option-card" href="/courses"><MapPinned aria-hidden="true" /><span><strong>Find your course</strong><small>Check layouts and plan where to play.</small></span><ChevronRight aria-hidden="true" /></Link><Link className="play-option-card" href="/play?eventId=flightforge-demo-event"><PlayCircle aria-hidden="true" /><span><strong>Try the field demo</strong><small>Practice scoring on a clearly labeled fictional round.</small></span><ChevronRight aria-hidden="true" /></Link></div>
      </section>
