...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const now = new Date();
  return ["", "/courses", "/events", "/community", "/places/new-england", "/roadmap", "/legal/privacy", "/legal/terms", "/legal/community-guidelines"].map((path) => ({ url: `${root}${path}`, lastModified: now }))
