...
export async function POST(request: Request) {
  ...
  try { const context=await getRoundAssistance(user,parsed.data.roundKey,parsed.data.holeNumber);
    if(parsed.data.roundKey&&!context)return apiError("ROUND_CONTEXT_UNAVAILABLE","This round context is no longer available. Return to your scorecard and reopen the caddie.",409);
    return Response.json(await sendCaddieMessage(user, parsed.data.message, parsed.data.conversationId,context), { status: 201 }); }
