...
import { resolveRoundContext } from "@/modules/rounds/round-context";
import {
  completeActiveRound,
...
export async function GET(request: Request) {
  ...
  if (!isPlayerReady(user)) return apiError("AUTHENTICATION_REQUIRED", "Sign in to synchronize this round.", 401);
  const event = await resolveRoundContext(user, new URL(request.url).searchParams.get("eventId") ?? "").catch(()=>null);
...
export async function PUT(request: Request) {
  ...
  const event = await resolveRoundContext(user, parsed.data.eventId).catch(()=>null);
  if (!event) return apiError("EVENT_NOT_FOUND", "The active event could not be loaded.", 404);
  try { return Response.json({ round: await saveHoleScore(user, event, parsed.data) }); }
  catch (error) {
    if(error instanceof RoundUnavailableError)return apiError("ROUND_UNAVAILABLE","This round is no longer active. Your unsynced draft is preserved on this device.",409);
...
export async function POST(request: Request) {
  ...
  const user = await getCurrentUser();
  if (!isPlayerReady(user)) return apiError("AUTHENTICATION_REQUIRED", "Sign in to finish and save this round.", 401);
