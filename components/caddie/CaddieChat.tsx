...
export function CaddieChat({roundContext}:{roundContext?:RoundAssistance|null}) {
  ...
  async function send(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/caddie/chat", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ message, conversationId, roundKey:roundContext?.roundKey, holeNumber:roundContext?.holeNumber }) });
  ...
  async function connectVoice() {
    ...
    try {
      ...
      const channel = peer.createDataChannel("oai-events"); dataRef.current=channel;
      ...
      const response = await fetch("/api/caddie/realtime"+(roundContext?"?roundKey="+encodeURIComponent(roundContext.roundKey)+"&holeNumber="+roundContext.holeNumber:""), { method: "POST", headers: { "content-type": "application/sdp" }, signal:AbortSignal.timeout(30_000), body: offer.sdp });
  ...
  return <section id="caddie-chat" className="caddie-chat" aria-labelledby="caddie-chat-title">
    ...
    {roundContext?<p className="caddie-chat-status">Using {roundContext.course}, hole {roundContext.holeNumber}. Distance and weather still need your input.</p>:null}
    ...
    <div className="caddie-voice-row"><label><input type="checkbox" checked={readAloud} onChange={(event) => setReadAloud(event.target.checked)} /><Volume2 aria-hidden="true" /> Read text replies</label>{voiceAvailable ? voiceState === "idle" || voiceState === "connecting" ? <button className="button button-secondary" type="button" onClick={connectVoice} disabled={voiceState === "connecting"}><Mic aria-hidden="true" />{voiceState === "connecting" ? "Connecting…" : "Connect live voice"}</button> : <button className={`voice-hold${voiceState === "talking" ? " is-listening" : ""}`} type="button" onPointerDown={beginTalking} onPointerUp={stopTalking} onPointerCancel={stopTalking} onKeyDown={event=>{if((event.key===" "||event.key==="Enter")&&!event.repeat){event.preventDefault();startTalking();}}} onKeyUp={event=>{if(event.key===" "||event.key==="Enter"){event.preventDefault();stopTalking();}}} onBlur={stopTalking}><Mic aria-hidden="true" />{voiceState === "talking" ? "Release to ask" : "Hold to talk"}</button> : <span className="voice-not-configured"><MicOff aria-hidden="true" />Live voice appears when the provider is configured.</span>}{voiceState!=="idle"&&voiceState!=="connecting"?<button type="button" className="button button-secondary" onClick={()=>{closeVoice();setVoiceState("idle");setStatus("Voice disconnected. Your microphone is off.");}}>Disconnect voice</button>:null}</div>
